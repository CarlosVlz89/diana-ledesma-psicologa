...
// --- SEGURIDAD: LISTA VIP ---
// Solo estos correos verán los botones de editar.
const adminEmails = [
  "carlos.mtzvelez@gmail.com", // Tu correo
  "dianaledesma@gmail.com"     // Correo de Diana (asegúrate de que sea el correcto de Gmail)
];

// --- UTILIDADES DE ANIMACIÓN (INTERACTIVIDAD) ---
...
const Button = ({ children, onClick, variant = "primary", className = "", type="button" }) => {
  // Estilos base + animación de "hundirse" al clic (active:scale-95)
  const baseStyle = "group relative px-6 py-3 rounded-full font-medium transition-all duration-300 transform active:scale-95 flex items-center justify-center gap-2 overflow-hidden";
  
  const variants = {
    primary: "bg-stone-800 text-white hover:bg-stone-700 shadow-lg hover:shadow-stone-500/30 hover:-translate-y-1",
    secondary: "bg-indigo-100 text-indigo-800 hover:bg-indigo-200 hover:-translate-y-1",
...
const Navbar = ({ user, isAdmin }) => {
  ...
  return (
    <nav className={`fixed w-full top-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 backdrop-blur-lg shadow-sm py-3' : 'bg-transparent py-6'}`}>
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <div className="relative">
              <Brain className={`h-8 w-8 transition-colors duration-300 ${scrolled ? 'text-indigo-600' : 'text-indigo-700'} group-hover:scale-110 transform transition-transform`} />
              <div className="absolute inset-0 bg-indigo-400 opacity-20 blur-lg rounded-full animate-pulse"></div>
            </div>
            <div>
...
const Hero = () => (
  <div id="home" className="relative overflow-hidden bg-stone-50 min-h-screen flex items-center pt-20">
    ...
    <div className="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 rounded-full bg-indigo-200 opacity-40 blur-3xl animate-pulse"></div>
    <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 rounded-full bg-sky-200 opacity-40 blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
    
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div className="space-y-8">
          <Reveal>
            <div className="inline-block px-4 py-1.5 bg-indigo-50 text-indigo-700 rounded-full text-sm font-semibold tracking-wide mb-2 shadow-sm border border-indigo-100">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-stone-800 leading-tight">
              Compromiso, aceptación y una <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">vida valiosa</span>.
            </h1>
          </Reveal>
        ...
        {/* Placeholder para Foto Principal con efecto flotante */}
        <Reveal delay={300}>
          <div className="relative mt-8 md:mt-0 group perspective-1000">
            <div className="aspect-[4/5] rounded-2xl overflow-hidden shadow-2xl bg-stone-200 relative group max-w-md mx-auto transform transition-transform duration-700 group-hover:rotate-1 group-hover:scale-[1.02]">
              <img 
            ...
            <div className="absolute -bottom-6 -right-4 md:-left-6 md:right-auto bg-white/95 backdrop-blur-md p-6 rounded-xl shadow-lg max-w-xs hidden md:block border border-stone-100 transform transition-all duration-500 hover:-translate-y-2">
            ...
            <div className="space-y-4 text-stone-700 leading-relaxed">
              ...
              <ul className="grid gap-3 mt-4">
                {[
                  ...
                  <li key={idx} className="flex items-start gap-3 bg-stone-50 p-3 rounded-lg border border-stone-100 hover:bg-stone-100 transition-colors">
       ...
       <div className="order-1 md:order-2 grid grid-cols-2 gap-4">
         <Reveal delay={300}>
           <div className="space-y-4">
              <div className="bg-stone-100 h-48 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="bg-indigo-100 h-64 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300">
         ...
         <Reveal delay={500}>
           <div className="space-y-4 pt-8">
              <div className="bg-sky-100 h-64 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((s, i) => (
            <Reveal key={i} delay={i * 100}>
              <div className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-500 border border-stone-100 group hover:-translate-y-2 h-full">
                ...
                <h3 className="text-xl font-bold text-stone-800 mb-3 group-hover:text-indigo-700 transition-colors">{s.title}</h3>
...
const Blog = ({ user, isAdmin }) => {
  ...
  useEffect(() => {
    // Usamos la ruta simplificada que ya tenías
    const q = query(
  ...
  const handleSavePost = async (e) => {
    e.preventDefault();
    if (!isAdmin) return; // Ahora usamos isAdmin para mayor seguridad
    setError('');
  ...
  return (
    <Section id="blog" className="bg-white">
      <Reveal>
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4 border-b border-stone-100 pb-8">
      ...
      {isEditing && isAdmin && (
        <div className="bg-stone-50 p-8 rounded-2xl shadow-xl border border-indigo-100 mb-12 animate-fade-in ring-4 ring-indigo-50">
          <h3 className="text-xl font-bold mb-6 text-stone-800 flex items-center gap-2"><Edit2 size={20} className="text-indigo-600"/> Editor de Artículos</h3>
          <form onSubmit={handleSavePost} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full p-3 border border-stone-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
      ...
      {loading ? (
        <div className="text-center py-20 text-stone-400 animate-pulse">Cargando pensamientos...</div>
      ) : posts.length === 0 ? (
        <div className="text-center py-20 bg-stone-50 rounded-2xl border-dashed border-2 border-stone-200">
           ...
           {isAdmin && <p className="text-indigo-600 mt-2 text-sm">¡Sé la primera en escribir!</p>}
        </div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.map((post, i) => (
            <Reveal key={post.id} delay={i * 100}>
              <article className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-stone-100 flex flex-col h-full group hover:-translate-y-1">
                <div className="h-48 bg-indigo-50 flex items-center justify-center overflow-hidden relative">
                   <div className="absolute inset-0 bg-white/20 group-hover:bg-transparent transition-colors"></div>
                   <span className="text-indigo-200 text-6xl opacity-50 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
                     {post.category === 'Ansiedad' ? <Brain size={64}/> : post.category === 'Pareja' ? <Heart size={64}/> : <BookOpen size={64}/>}
                   </span>
                </div>
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-3">
                     <span className="text-xs font-bold tracking-wider text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full uppercase shadow-sm">
                       {post.category}
                     </span>
                     {isAdmin && (
                       <div className="flex gap-2">
                         <button onClick={() => { setCurrentPost(post); setIsEditing(true); }} className="p-2 hover:bg-stone-100 rounded-full text-stone-400 hover:text-indigo-600 transition-colors"><Edit2 size={16}/></button>
                         <button onClick={() => handleDelete(post.id)} className="p-2 hover:bg-red-50 rounded-full text-stone-400 hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
                       </div>
                     )}
                  </div>
                  <h3 className="text-xl font-bold text-stone-800 mb-3 group-hover:text-indigo-700 transition-colors">{post.title}</h3>
  ...
  <div className="bg-gradient-to-br from-stone-50 to-indigo-50/30">
    <Section id="contact">
      <div className="grid lg:grid-cols-2 gap-16">
        ...
        <Reveal delay={200}>
          <div className="bg-white rounded-3xl shadow-xl p-8 border border-stone-100 relative overflow-hidden transform hover:-translate-y-2 transition-transform duration-500">
            ...
            <div className="bg-indigo-50 rounded-xl p-6 text-center mb-8 border border-indigo-100">
              <Calendar className="w-12 h-12 text-indigo-600 mx-auto mb-3 animate-bounce" style={{ animationDuration: '3s' }}/>
            ...
            <div className="border-t border-stone-100 pt-6">
              <h4 className="font-bold text-stone-800 mb-2">Dudas rápidas</h4>
               <ul className="text-sm text-stone-600 space-y-2">
                 <li className="flex items-center gap-2"><CheckCircle size={14} className="text-green-500"/> Duración de sesión: 50 minutos.</li>
                 <li className="flex items-center gap-2"><CheckCircle size={14} className="text-green-500"/> Modalidad: Presencial y Online (Zoom).</li>
                 <li className="flex items-center gap-2"><CheckCircle size={14} className="text-green-500"/> Pago: Efectivo o Transferencia.</li>
               </ul>
            </div>
          </div>
        </Reveal>
...
const Footer = ({ onOpenPrivacy, handleLogin, user, isAdmin, handleLogout }) => (
      ...
      <div>
        ...
        {user ? (
          <div>
            ...
            {isAdmin && <p className="text-xs text-green-400 mb-2">★ Administrador</p>}
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen ${colors.bg} font-sans text-stone-800 selection:bg-indigo-100 selection:text-indigo-900`}>
      <Navbar user={user} isAdmin={isAdmin} />
      
      <main className="animate-fade-in">
        {/* Todas las secciones en una sola página con animaciones */}
        <Hero />
        <About />
        <Services />
